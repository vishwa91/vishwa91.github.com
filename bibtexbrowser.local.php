...
<!--this is version v924f5e53a230306f973bc29a69fc17d185d2a463 -->
...
define('BIBTEXBROWSER','v924f5e53a230306f973bc29a69fc17d185d2a463');
...
@define('BIBTEXBROWSER_DEFAULT_FRAME','all'); // year=latest,all and all valid bibtexbrowser queries
...
@define('ORDER_FUNCTION_FINE','compare_bib_entry_by_bibtex_order');
...
@define('BIBTEXBROWSER_BIBTEX_VIEW_FILTEREDOUT','comment|note|file|url_Slides|pdf|url_video|pubtype');
...
@define('USE_FIRST_THEN_LAST',true); // use only initials for all first names?
...
@define('YEAR_SIZE',15); // number of years per table
@define('AUTHORS_SIZE',0); // number of authors per table
...
class BibEntry {
  ...
  function transformValue($value) {
    if (c('BIBTEXBROWSER_USE_LATEX2HTML'))
    {
        ...
        $value = html_entity_decode($value, ENT_QUOTES, OUTPUT_ENCODING);
    }
    return $value;
  }
...
//     echo $this->getKey();
  ...
  */
  ...
  function getSlidesLink($iconurl = NULL, $label = 'ppt') {
    if ($this->hasField('url_Slides')) {
      return $this->getLink('url_Slides', $iconurl, $label);
    }
    return "";
  }
    function getCodeLink($iconurl = NULL, $label = 'code') {
        if ($this->hasField('code')) {
            return $this->getLink('code', $iconurl, $label);
        }
        return "";
    }
    function getVideoLink($iconurl = NULL, $label = 'vid') {
        if ($this->hasField('url_video')) {
            return $this->getLink('url_video', $iconurl, $label);
        }
        return "";
    }

    function getWebLink($iconurl = NULL, $label = 'project') {
        if ($this->hasField('url_web')) {
            return $this->getLink('url_web', $iconurl, $label);
        }
        return "";
    }

    function getSuppLink($iconurl = NULL, $label = 'supp') {
        if ($this->hasField('pdf_supp')) {
            return $this->getLink('pdf_supp', $iconurl, $label);
        }
        return "";
    }
  ...
  */
  ...
  function toEntryUnformatted() {
    ...
    $entry = htmlspecialchars($this->getFullText(),ENT_NOQUOTES, OUTPUT_ENCODING);
...
function bib2links_default(&$bibentry) {
  ...
  if (BIBTEXBROWSER_PDF_LINKS) {
    ...
    if (BIBTEXBROWSER_SUPP_LINKS) {
        $link = $bibentry->getSuppLink();
        if ($link != '') { $links[] = $link; };
    }

  if (BIBTEXBROWSER_SLIDES_LINKS) {
    $link = $bibentry->getSlidesLink();
    if ($link != '') { $links[] = $link; };
  }
    ...
    if (BIBTEXBROWSER_WEB_LINKS) {
        $link = $bibentry->getWebLink();
        if ($link != '') { $links[] = $link; };
    }
  ...
  return '<br><span class="bibmenu">'.implode(" ",$links).'</span>';
}
...
if (!function_exists('poweredby')) {
  ...
  function poweredby() {
    ...
    $poweredby .= 'Powered by <a href="http://www.monperrus.net/martin/bibtexbrowser/">bibtexbrowser</a><!--v924f5e53a230306f973bc29a69fc17d185d2a463-->';
    $poweredby .= '</div>'."\n";
    return $poweredby;
  }
}
...
function query2title(&$query) {
	...
	$v = join($v, ',');
      ...
      $headers[$k] = __(ucwords($k)).': '.ucwords(htmlspecialchars($v,ENT_NOQUOTES, OUTPUT_ENCODING));
  }
  return join(' &amp; ',$headers);
}
} // if (!function_exists('query2title'))
  ...
  function display() {
    ...
    if ($this->headingLevel == BIBTEXBROWSER_HTMLHEADINGLEVEL) {
      ...
      if (count($this->entries) == 1) {
        echo count ($this->entries).' '.__('zzz result');
      } else if (count($this->entries) != 0) {
        echo count ($this->entries).' '.__('aaa results');
      }
      echo "</span>\n";*/
...
<meta name="generator" content="bibtexbrowser v924f5e53a230306f973bc29a69fc17d185d2a463" />
<?php
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
   <channel>
      ...
      <generator>bibtexbrowser v924f5e53a230306f973bc29a69fc17d185d2a463</generator>
...
class Dispatcher {
  ...
  function diagnosis() {
    ...
    echo "bibtexbrowser version: 924f5e53a230306f973bc29a69fc17d185d2a463\n";
  ...
  function frameset() {    ?>
    ...
    <frameset cols="15%,*">
    <frame name="menu" frameborder="0" src="<?php echo '?'.Q_FILE.'='. urlencode($_GET[Q_FILE]).'&amp;menu'; ?>" />
    <frame name="main" frameborder="0" src="<?php echo '?'.Q_FILE.'='. urlencode($_GET[Q_FILE]).'&amp;'.BIBTEXBROWSER_DEFAULT_FRAME?>" />
    </frameset>
    </html>
